...
└── Hyperfocus/
    ...
    │   └── Stats/
    │       ├── StatsScreen.swift           # 통계 화면 컨테이너 (Today → This Week → This Month → Past Days)
    ...
    │       ├── MonthlyCalendarView.swift   # This Month 섹션 — 월간 캘린더 (탭으로 현재 달 이동)
...
- `TimerStore.onBlockingStart` → `FocusBlocker.activate(blocklist:)` 연결
- `TimerStore.onBlockingStop` → `FocusBlocker.deactivate()` 연결
- `FocusStore.onStateChanged` → `FocusBlocker.updateBlocklist(_:)` 호출 + `Persistence.requestSave()`
...
- 파생: `currentBlocklist: FocusBlocklist` — 현재 상태를 `FocusBlocklist` 값으로 반환
- 콜백: `onStateChanged: (() -> Void)?` — CRUD 발생 시 `HyperfocusApp`이 `FocusBlocker.updateBlocklist()` + `Persistence.requestSave()` 실행
...
- `StatsScreen`은 위에서 아래로 **요약 영역 → Today 섹션(`CurrentDayCardView`) → This Week 섹션(`WeeklyBarChartView`) → This Month 섹션(`MonthlyCalendarView`) → Past Days 섹션(`PastDayRowView` 목록)** 순서로 배치한다. 섹션 헤더 라벨은 각각 `"Today"`, `"This Week"`, `"This Month"` 로 표시되며, 섹션 헤더 위에는 12pt padding.top 간격을 준다.
...
- `CurrentDayCardView`는 `DayCard`를 사용하여 단순화된다. `aggregatedSessionsIncluding(day:active:)`로 세션 목록을 가져와 `DayCard`에 전달한다. 편집·삭제 액션은 `TimerStore.renameSession(id:to:)` / `deleteSession(id:)`를 호출한다. 삭제 확인 UI는 `[Delete]` / `[Cancel]` 버튼만 표시한다(텍스트 메시지 없음, 버튼 왼쪽 정렬). `activeSession`은 편집/삭제 대상에 포함되지 않으며, 삭제 후 세션이 0개가 되어도 현재 하루 카드는 사라지지 않는다.
- `DayDetailView`는 `DayCard`를 사용하여 Today 카드와 동일한 레이아웃으로 표시한다. 네비게이션 바 타이틀은 `yyyy-MM-dd` 형식의 날짜이며, 뒤로 가기 버튼 라벨은 `"Stats"`. `statsStore.pastDays`에서 `day.id`로 최신 Day를 조회하여 `aggregatedSessions(of:)`로 표시한다. 삭제 확인 UI는 `[Delete]` / `[Cancel]` 버튼만 표시한다(텍스트 메시지 없음, 버튼 왼쪽 정렬). day가 삭제되어 `pastDays`에서 사라지면 자동으로 `onBack()` 호출.
- `WeeklyBarChartView`는 이번 주(월~일) 날짜별 작업 시간을 바 차트로 표시한다. 차트/캘린더 내 폰트는 10pt(소), 11pt(중) 크기를 사용한다.
- `MonthlyCalendarView`는 이번 달의 날짜별 작업 기록을 캘린더 형태로 표시한다. 캘린더 상단 월 타이틀(예: `May 2026`)을 탭하면 현재 달(오늘이 속한 달)로 이동한다. `onSelectDay: (Day) -> Void` 콜백을 파라미터로 받아, 날짜 셀을 탭했을 때 `StatsScreen`을 거쳐 `PopoverRoot`로 선택된 날짜를 전달하여 `DayDetailView`로 이동한다.
...
DayDetailView (편집 아이콘 클릭 → Return 키)
  ...
  → @Observable 변경으로 DayDetailView 재렌더링 (통합 합산 뷰 갱신)
```
...
CurrentDayCardView (편집 아이콘 클릭 → Return 키)
  → TimerStore.renameSession(id: id, to: editingText)
    ...
    - onStateChanged?() → Persistence.requestSave()
  → @Observable 변경으로 CurrentDayCardView 재렌더링 (합산 뷰 갱신)
```
...
| 과거 세션 편집/삭제 동작 변경                                                      | `Stores/StatisticsStore.swift`(`renameSession`/`deleteSession` 로직), `Views/Stats/DayDetailView.swift`(UI), SPEC §4.3 |
