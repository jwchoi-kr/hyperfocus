...
세션이 실행되는 동안 방해 앱을 자동으로 종료하고, Chrome에서 특정 사이트를 차단하는 "Focus Mode" 기능을 Hyperfocus에 추가한다. 설정은 Popover의 새 Focus 탭에서 관리하며, 한 번 설정하면 앱 재시작 후에도 유지된다.
